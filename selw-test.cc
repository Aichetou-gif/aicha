...
RoutingExperiment::RoutingExperiment ()
  : port (9),
    ...
    packetsReceived (0),
    m_CSVfileName ("selw-routing.output.csv"),
...
RoutingExperiment::CheckThroughput ()
{
 ...
 double S = k + PDR;
  //std::cout <<(Simulator::Now()).GetSeconds () << "S = " << k << "+" << PDR <<"="<< (k + PDR) <<std:: endl;
    k = S;
   //std::cout << "Somme = " << k <<std:: endl;
double St = kt + kbs;
    kt = St;
 ...
 */
  ...
  for (auto it = ueVoiceNetDev.Begin (); it != ueVoiceNetDev.End (); ++it)
    ...
    for (const auto &it:bwpIdContainer)
      ...
      slUeSelectedPreConfig.slProbResourceKeep = 0.5;
...
*/
        ...
        std::cout << "Avrg PDR = " << k /12 <<std:: endl;
        std::cout << "Avrg thput = " << kt /12 <<std:: endl;
  Simulator::Destroy ();
//  return 0;
}
